...
use crate::{either::Either, r#as::AsWrap, CellMarker, ResultExt};
...
impl<'a, C, T, As> CellSerializeAs<C, &'a mut T> for &'a mut As
where
    As: CellSerializeAs<C, T> + ?Sized,
    T: ?Sized,
{
...
impl<C, T, As> CellSerializeAs<C, [T]> for [As]
where
...
impl<C, T, As, const N: usize> CellSerializeAs<C, [T; N]> for [As; N]
where
    As: CellSerializeAs<C, T>,
{
    #[inline]
...
macro_rules! impl_cell_serialize_as_for_tuple {
    ($($n:tt:$t:ident as $a:ident),+) => {
        impl<C, $($t, $a),+> CellSerializeAs<C, ($($t,)+)> for ($($a,)+)
        where $(
            $a: CellSerializeAs<C, $t>,
        )+
        {
            #[inline]
            fn store_as(source: &($($t,)+), builder: &mut CellBuilder<C>) -> Result<(), CellBuilderError<C>> {
...
impl<C, T, As> CellSerializeAs<C, Box<T>> for Box<As>
where
    As: CellSerializeAs<C, T> + ?Sized,
{
    #[inline]
...
impl<C, T, As> CellSerializeAs<C, Rc<T>> for Rc<As>
where
    As: CellSerializeAs<C, T> + ?Sized,
{
    #[inline]
...
impl<C, T, As> CellSerializeAs<C, Arc<T>> for Arc<As>
where
    As: CellSerializeAs<C, T> + ?Sized,
{
    #[inline]
...
/// ```
impl<C, Left, Right, AsLeft, AsRight> CellSerializeAs<C, Either<Left, Right>> for Either<AsLeft, AsRight>
where
    AsLeft: CellSerializeAs<C, Left>,
    AsRight: CellSerializeAs<C, Right>,
{
    #[inline]
    fn store_as(
...
impl<C, T, As> CellSerializeAs<C, Option<T>> for Either<(), As>
where
    As: CellSerializeAs<C, T>,
{
    #[inline]
...
/// ```
impl<C, T, As> CellSerializeAs<C, Option<T>> for Option<As>
where
    As: CellSerializeAs<C, T>,
{
    #[inline]
...
pub trait CellSerializeWrapAsExt {
    #[inline]
    fn wrap_as<C, As>(&self) -> AsWrap<&'_ Self, As>
    where
        As: CellSerializeAs<C, Self> + ?Sized,
    {
        AsWrap::new(self)
    }
}
